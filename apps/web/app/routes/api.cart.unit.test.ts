...
describe('api.cart loader', () => {
	it('clears an orphaned cart count without loading a cart session', async () => {
		const request = new Request('https://example.com/api/cart', {
			headers: { Cookie: 'cart_count=3' },
		});
		...
		expect(response.headers.get('Set-Cookie')).toBe('cart_count=; Path=/; SameSite=Lax; Max-Age=0');
