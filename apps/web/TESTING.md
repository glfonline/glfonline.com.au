...
This project uses **Test-Driven Development (TDD)** with **Vitest** for unit tests and **Vitest Browser Mode** for component tests, using **co-located tests** for better developer experience.
...
describe('sortSizes', () => {
  it('should sort sizes correctly', () => {
    expect(sortSizes(['L', 'S', 'M'])).toEqual(['S', 'M', 'L']);
  });
});
...
// 3. Add more tests and refine implementation
```
